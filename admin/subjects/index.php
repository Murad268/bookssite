...
$toplamKayitSayisiSorgusu = $dbh->prepare("SELECT * FROM subjects $s");
...
$getSubjects = $dbh->prepare("SELECT * FROM subjects $s LIMIT $sayfalamayBaslayacaqKayotSayisi, $sayfaBasinaGosterilecek");
$getSubjects->execute();
...
<div class="langs__content">
   <div class="mt-4 ms-4 author__content__form">
      <form method="POST" action="./server/process.php">
         ...
         <button type="submit" name="searchSubjects" class="btn btn-dark">axtar</button>
      </form>
   </div>
         ...
         <div style="margin: 20px auto" class="col-xl-6">
            <div class="bg-secondary rounded h-100 p-4">
               <form method="POST" action="./server/process.php">
                  ...
                  <button name="add__subject" type="submit" class="btn btn-primary">Əlavə et</button>
               </form>
            </div>
         </div>
      </div>
