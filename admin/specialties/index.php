...
$toplamKayitSayisiSorgusu = $dbh->prepare("SELECT * FROM specialties $s");
...
$getSpecialties = $dbh->prepare("SELECT * FROM specialties $s LIMIT $sayfalamayBaslayacaqKayotSayisi, $sayfaBasinaGosterilecek");
$getSpecialties->execute();
$classes = $getSpecialties->fetchAll(PDO::FETCH_ASSOC);
?>
<div class="langs__content">
   <div class="mt-4 ms-4 author__content__form">
      <form method="POST" action="./server/process.php">
         ...
         <button type="submit" name="searchSpeciality" class="btn btn-dark">axtar</button>
      </form>
   </div>
               ...
               <table class="table">
                  ...
                  <tbody>
                     ...
                     foreach ($classes as $class) {
                        ...
                        $getLang->execute([$class["lang_id"]]);
                        $lang = $getLang->fetch(PDO::FETCH_ASSOC); ?>
                        <tr>
                           <th scope="row"><?php echo $class["id"] ?></th>
                           <td><?php echo $class["name"] ?></td>
                           <td><?php echo $lang["name"] ?></td>
                           <td><a onclick="return confirm('Dili silmək istədiyinizdən əminsinizmi?')" href="./server/process.php?specialityproc=delete&id=<?php echo $class["id"] ?>" class=""><i class="fa fa-window-close"></i></a></td>
                        </tr>
                     <?php
                     }
                     ?>
         ...
         <div style="margin: 20px auto" class="col-xl-6">
            <div class="bg-secondary rounded h-100 p-4">
               <form method="POST" action="./server/process.php">
                  ...
                  <div class="mb-3">
                     <input type="text" name="speciality_name" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp">
                  </div>
                  <button name="add__specialities" type="submit" class="btn btn-primary">Əlavə et</button>
               </form>
            </div>
         </div>
      </div>
